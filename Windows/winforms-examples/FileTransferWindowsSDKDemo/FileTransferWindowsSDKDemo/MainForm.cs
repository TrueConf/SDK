...
{    
    public partial class MainForm : Form
    {
        ...
        public MainForm(string server, string id, string password)
        {
            ...
            files.Columns.Add("PeerID", typeof(string));
            files.Columns.Add("Path", typeof(string));
            listFiles.AutoGenerateColumns = true;
            listFiles.DataSource = files;
            listFiles.Columns[0].Visible = false;
            //
        }
        ...
        private void UpdateFileList()
        {
            ...
            {
                if(!files.Rows.Contains((int)file.id))
                {
                    files.Rows.Add((int)file.id, (string)file.fileName, (string)file.peerId, (string)file.downloadDir);
                }
            }
    ...
    public class ChatFile
    {
        ...
        public ChatFile(dynamic file)
        {
            Name = file.fileName;
            PeerID = file.peerId;
            Path = file.downloadDir;
        }
